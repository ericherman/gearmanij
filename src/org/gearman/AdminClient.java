...
public interface AdminClient {
     ...
     * strings in the list are blank space delimited. Each string begins with
     * the file descriptor, IP address, the worker ID, followed by a colon. If a
     * worker has not set an ID a '-' appears in place of the ID. The colon is
     * followed by a list of registered functions a worker can perform. The
     * format is:
     * <p>
    ...
    /**
     * Returns status information about each function registered with a job
     * server by at least one worker. The strings in the list are tab delimited.
     * The format is:
     * <p>
     * FUNCTION\tTOTAL\tRUNNING\tAVAILABLE_WORKERS
     * 
     * @return status information about each function registered with a job
     *         server
     */
    List<String> getFunctionStatus();

    /**
     * Sets the maximum queue size for a function to the default size for the
     * job server.
     * 
     * @param functionName
